...
pcd_folder = "/home/bk/Study/RnD/HBRS-dataset/outputs/run_2023_12_02_14_55_38/pcd/"
image_folder = "/home/bk/Study/RnD/HBRS-dataset/outputs/run_2023_12_02_14_55_38/jpg/"
cloud = [pcl.load_XYZI(os.path.join(pcd_folder, f)) for f in sorted(os.listdir(pcd_folder))]

images = [cv2.imread(os.path.join(image_folder, f)) for f in sorted(os.listdir(image_folder))]
print(len(cloud), len(images))
...
for j, cloud in enumerate(cloud):
    image = images[j]
    ...
    cv2.imwrite(f"output/point_cloud_{j}.png", image)
